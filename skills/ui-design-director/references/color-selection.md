...
Before approval, test intended foreground/background pairs rather than judging swatches in isolation. Report pass/fail for normal text, large text, and essential controls as relevant. Adjust the role mapping or derive an accessible shade when the chosen raw color fails; preserve the user's selected color as an accent if appropriate.
